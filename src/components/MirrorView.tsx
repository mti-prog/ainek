...
export default function MirrorView() {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden" style={{ background: "#06060f" }}>
      ...
      {generatedImage && (
        <div className="absolute inset-0 transition-opacity duration-700 ease-in-out"
          ...
          <Image src={generatedImage} alt="Virtual try-on result" fill
            className="object-cover" sizes="100vw" priority unoptimized />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 z-10"
          ...
          <div className="relative w-64 h-64">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <Zap size={32} className="text-violet-400 mb-2"
      ...
      {/* BOTTOM BAR */}
      <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-8 py-6 z-20"
        ...
        {fittingState === "result" && (
          <div className="flex items-center gap-3">
            ...
            <button
              className="flex items-center gap-2 px-6 py-3 rounded-xl text-white text-sm font-medium touch-manipulation"
              style={{ background: "linear-gradient(135deg, #8b5cf6, #3b82f6)",
                boxShadow: "0 4px 20px rgba(139,92,246,0.4)" }}>
              <ShoppingBag size={16} />
              {selectedProduct?.price || "Примерить ещё"}
            </button>
          </div>
        )}
        {fittingState === "idle" && (
